function Remove-PSADOProject {
    ...
    Process {
        $ProjectID = (Get-PSADOProject -CompanyName $CompanyName -ProjectName $ProjectName -User $User -token $token).id
        [uri]$uri = "https://dev.azure.com/$CompanyName/_apis/projects/$($projectID)?api-version=5.0"
        if ($PSCmdlet.ShouldProcess(
                ("Project {0} wil be deleted" -f $ProjectName),
                ("This action will DELETE project {0}, do you want to continue?" -f $ProjectName),
                "Project deletion"
            )
        ) {
